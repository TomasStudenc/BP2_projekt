...
def GA(cords, params):
    ...
    seed = int(params.get("GA_seed",
                          173))  # seed na ktorom beží random aby pri testovaní sme sledovali rovnakú náhodnosť pri menení parametrov
    ...
    def crossover(p1, p2):  # vstupujú dvaja členovia populácie
        ...
        child1_genome = p1.genome[:cut] + [g for g in p2.genome if g not in p1.genome[
                                                                            :cut]]  # poskladá nový genom tak aby bola prvá časť z p1 a druhá z p2
        child2_genome = p2.genome[:cut] + [g for g in p1.genome if g not in p2.genome[
                                                                            :cut]]  # poskladá nový genom tak aby bola prvá časť z p2 a druhá z p1
    ...
    for g in range(generations):
        ...
        while len(new_population) < pop_size:  # vytváranie zvyšku populácie
            p1, p2 = tournament_selection(population), tournament_selection(
                population)  # výber dvoch rodičov pre kríženie
        ...
        current_best = min(population,
                           key=lambda p: p.fitness)  # nájednie najlepšieho lokálneho riešenia v danej generácií

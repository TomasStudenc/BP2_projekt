...
def CSA(cords, params):
    ...
    class Nest():
        def __init__(self):
            self.genome = rnd_csa.sample(range(len(cords)),len(cords))
    ...
    def flight(genome, step_size):
        ...
        u = rnd_csa.gauss(0, sigma)
        v = rnd_csa.gauss(0, 1)
        step = abs(u/(abs(v)**(1/beta)))
        swap_count = max(1,int(step_size*step*len(genome)*0.1))
        swap_count = min(swap_count, len(genome)//2)
        for _ in range(swap_count):
            x1, x2 = rnd_csa.sample(range(len(genome)),2)
            new_genome[x1], new_genome[x2] = new_genome[x2] , new_genome[x1]
        return new_genome

    population = [Nest() for _ in range(nest_count)]
    best_nest = min(population, key= lambda x: x.fitness)
    global_best = best_nest.fitness
    best_genome = best_nest.genome[:]
    best_distance = []

    for g in range(generations):
        for cuckoo in population:
            new_genome = flight(cuckoo.genome, step_size)
            new_fitness = route_length(new_genome, cords)
            random_nest = random.choice(population)
            if new_fitness < random_nest.fitness:
                random_nest.genome = new_genome
                random_nest.evaluate(cords)
            abandon_count = max(1 , int(pa*nest_count))

...
def ACO(cords, params):
    ...
    class Environment:
        ...
        def __init__(self):
            ...
            self.distance = [[math.dist(cords[i], cords[j]) if i != j else 0 for j in range(self.cities)] for i in
                             range(self.cities)]
            # vytvorenie matice feromónov ktorá ukladá všetky feromónové ohodnotenia medzi vrcholmi
            self.pheromones = [[1 / (self.distance[i][j] * self.cities) if i != j else 0 for j in range(self.cities)]
    ...
    def construct_tour(env):
        ...
        while unvisited:  # kým existujú nenavštívené vrchli
            ...
            next_city = rnd_aco.choices(unvisited, weights=probs)[
                0]  # vyberie dalšei vrchol na základe pravdepodobnosti

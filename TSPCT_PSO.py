...
def PSO(cords, params):
    ...
    def apply_swaps(source, target, intensity):
        ...
        num_swaps = max(1, int(intensity * len(
            source)))  # vyber počtu zmien, čim viac zmien tým sa viac blížime k target pozicií
        for _ in range(num_swaps):
            diff_positions = [i for i in range(len(source)) if
                              new[i] != target[i]]  # najde pozície kde sa sorce a taget líššia
